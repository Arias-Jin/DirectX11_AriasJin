...
namespace arias
{
    ...
    void Time::Update()
    {
        ...
        float differenceInFrequancy = static_cast<float>((mCurFrequency.QuadPart - mPrevFrequency.QuadPart));

        mDeltaTime = differenceInFrequancy / static_cast<float>(mCpuFrequency.QuadPart);
        mBulletTime = (differenceInFrequancy / static_cast<float>(mCpuFrequency.QuadPart)) * 0.1f;
