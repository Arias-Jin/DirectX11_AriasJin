...
void main(uint3 DTid : SV_DispatchThreadID)
{
    ...
    if (ParticleBuffer[DTid.x].active == 0)
    {
        ...
        if (ParticleBuffer[DTid.x].active)
        {
            ...
            Random = float4(GaussianBlur(UV + float2(0.0f, 0.0f)).x, GaussianBlur(UV + float2(0.1f, 0.0f)).x, GaussianBlur(UV + float2(0.2f, 0.0f)).x, GaussianBlur(UV + float2(0.2f, 0.0f)).x);
